...
exports.handler = function(event, context, callback) {
    ...
    org.authenticate({ username: process.env.SFDC_USERNAME, password: process.env.SFDC_PASSWORD }, function(err, oauth) {
        ...
        var dominantLangParams = {
          Text: payload.detail.Message__c
        };
        ...
        dominantLangPromise.then(function(language) {
            ...
            Promise.all([detectEntitiesPromise, detectSentimentPromise]).then(function(data) {
                ...
                lead.set('Id', payload.detail.Record_Id__c);
                ...
                var q = 'SELECT Id FROM Lead_Entity__c WHERE Lead__c = \'' + payload.detail.Record_Id__c + '\' LIMIT 200';
                org.query({ query: q }, function(err, resp){
                  ...
                  if(!err && resp.records) {
                    ...
                    org.deleteUrl( { url: '/services/data/v52.0/composite/sobjects?ids=' + deleteIds } , function(err, resp){
                        if(err) console.log(err, err.stack); // an error occurred
                    });
                  }
                });
                ...
                for (var entity of data[0].Entities) {
                    ...
                    leadEntity.set('Lead__c', payload.detail.Record_Id__c);
                ...
                org.postUrl( { url: '/services/data/v52.0/composite/sobjects/' , body : { records : leadEntities } } , function(err, resp){
                    if(err) console.log(err, err.stack); // an error occurred
                });
